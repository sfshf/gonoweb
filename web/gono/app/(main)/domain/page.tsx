...
import { listDomain, addDomain, editDomain, deleteDomain } from "@/api/domain";
...
const AddDomain = ({
  ...
  const onPressConfirm = async () => {
    try {
      const resp: any = await addDomain({
        ...state,
      });
      addToast({
...
const EditDomain = ({
  ...
  const onPressConfirm = async () => {
    try {
      ...
      const resp: any = await editDomain({
        ...state,
        xid: domain.xid,
      });
      addToast({
...
const DeleteDomain = ({
  ...
  const onPressConfirm = async () => {
    try {
      ...
      const resp: any = await deleteDomain(domain.xid);
      addToast({
        title: t("app.prompt.ok"),
        description: resp.msg,
        color: "success",
...
export default function DomainPage() {
  ...
  const columns = [
    ...
    {
      key: "intro",
      label: "domain.label.intro",
    },
    {
      key: "created_at",
      label: "domain.label.createdAt",
    },
    {
      key: "updated_at",
      label: "domain.label.updatedAt",
    },
    {
      key: "operation",
      label: "domain.label.operation",
    },
  ];
  const reducer = (state: any, action: any) => {
    switch (action.type) {
  ...
  const searchDomain = async (page: number) => {
    try {
      setIsLoading(true);
      const resp: any = await listDomain({
        page,
  ...
  const onPressDelete = (domain: TDomain) => () => {
    setDomain(domain);
    onOpenDelete();
  };
  const onCloseDeleteDomain = () => {
    onCloseDelete();
    searchDomain(state.page);
  };
  return (
        ...
        <>
          <div className='flex flex-col items-center justify-center text-center'>
            <div className='w-[100%] flex flex-row flex-wrap text-start m-1'>
              {hasWidget(siteWidgets.btn_search_domain) && (
                <>
                  <Input
                  ...
                  {hasWidget(siteWidgets.btn_add_domain) && (
                    <Button
